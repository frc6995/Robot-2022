...
public class MidtakeCommandFactory {
  ...
  /**
   * Spins the mitake. Gets interrupted when the bottom beam is clear or when the
   * top beam is broken
   * 
   * @param midtakeS
   * @return load midtake
  ...
  /**
   * Waits until the bottom beam is broken and the top is clear, then loads until
   * the bottom is clear or the top is broken.
   * 
   * Has the effect of processing balls as the intake brings them in, and stopping
   * the balls as low as possible.
   * 
   * @param midtakeS
   * @return
   */
  ...
  /**
   * If a ball is breaking the top beam, drives the ball down until it breaks the
   * bottom beam.
   * 
   * @param midtakeS
   * @return
  ...
  public static Command createMidtakeArmC(MidtakeS midtakeS) {
     ...
     * It ensures the next ball to fire is stopped below the top beam break, but
     * just barely breaking it.
     * If no ball is actually in the intake, it will try to arm for roughly a second
     * before timing
     */
  ...
  public static Command createMidtakeFeedOneHoldingTwoC(MidtakeS midtakeS) {
    return new RunEndCommand(() -> midtakeS.spin(Constants.MIDTAKE_FEEDING_SPEED + 0.1), midtakeS::stop, midtakeS)
        .withInterrupt(midtakeS::getIsTopBeamClear)
        .withTimeout(0.1).andThen(createMidtakeReverseC(midtakeS).withTimeout(0.1));
  }
  ...
  /**
   * By default, the midtake will perpetually drive slowly towards the top except
   * when the top beam is broken,
   * to ensure that if a ball is in the midtake, it is situated up against the top
   * beam, ready to fire.
   * 
   * @param midtakeS
   * @return
  ...
  public static Command createMidtakeIdleC(MidtakeS midtakeS) {
    return new ConditionalCommand(
        ...
        createMidtakeCrawlC(midtakeS).withInterrupt(midtakeS::getIsTopBeamBroken)
            .andThen(new InstantCommand(midtakeS::reportArmed)),
        midtakeS::getIsTopBeamBroken);
  }
